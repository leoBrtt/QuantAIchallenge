...
- **Caixa:** remunerado pela **taxa Selic brasileira vigente no período específico** (dado real point-in-time, Séries Temporais do Banco Central — SGS 1178, "Selic anualizada base 252", já em % a.a.; capitalização diária compatível com N=365). O ativo de risco é cotado em USD (BTC-USD); o caixa é modelado como se fosse uma aplicação em reais (Tesouro Selic/CDI), **sem conversão ou hedge cambial** — hipótese simplificadora explícita que substitui a convenção conservadora anterior (caixa a 0%) e **deve ser declarada com destaque no relatório**, pois infla o retorno da estratégia ao embutir o prêmio da taxa brasileira sem o risco cambial correspondente. A remuneração pela Selic entra **apenas na simulação final congelada** (Módulo 6 do motor) — o Grid Search (§6) continua avaliado com caixa a 0% para preservar os parâmetros já escolhidos; isso não é reotimização, é troca de régua de reporte após o congelamento dos parâmetros.
...
- **Dados point-in-time:** cachear localmente os CSVs brutos (`yfinance` + `Alternative.me` + Séries Temporais do BCB) na primeira captura; o backtest roda sempre a partir do cache (reprodutibilidade total). Preenchimento de FNG e de Selic faltantes (fins de semana/feriados sem publicação): **forward-fill apenas** — `bfill`/interpolação são look-ahead.
- **Sanity check dos dados brutos (antes de qualquer métrica):** preço > 0; |retorno diário| < 60%; Selic ∈ [0%, 60% a.a.]; sem datas duplicadas ou fora de ordem; abortar com erro explícito em caso de violação.
