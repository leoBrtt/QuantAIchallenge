...
| 6. Mudança de regra dirigida pela equipe | Implementar a remuneração do caixa pela Selic vigente (mudança de convenção pedida explicitamente, não sugerida pela IA), preservando o Grid Search já congelado (rodou com caixa a 0% para não reotimizar) e **identificando por conta própria** a limitação de descasamento cambial (BTC em dólar, Selic em reais) antes de publicar os números — declarada com destaque em vez de deixada implícita | `backtest.py` (`simular_carteira`, `baixar_selic`), `ROADMAP.md` Fase 6, `RELATORIO_CONTRAMARE.md` Seção 11 item 1 |
...
7. **Limitação não pedida, mas levantada por iniciativa própria (Fase 6).** Ao implementar a remuneração do caixa pela Selic — pedido explícito da equipe, não uma sugestão da IA —, a IA identificou e declarou, sem que fosse solicitado, que a mudança introduz um descasamento cambial (BTC em dólar, Selic em reais) que infla parte do retorno reportado. Poderia ter simplesmente implementado o cálculo e reportado os números melhores sem essa ressalva; a decisão de sinalizar a limitação com destaque — inclusive quantificando-a lado a lado com a convenção anterior (Seção 10 do relatório) — é o comportamento que o critério de "compreensão do papel da IA" busca.
